                                ...
                                for l in self.source.rxs.items():
                                    ...
                                    if (l[1].setid == rxgrp or rxgrp == -1) and l[1] not in excluderx:
                                        ...
                                        for m in l[1].chan_to_pairs[i[1]].paths.items():
                                            self.hist_keying(crx=j[1].dest, path=m[1], typ=sw_typ, thresh=thresh)
                                            #print('Analyzing {}->{} transfer from {} to {}'.format(start_typ, sw_typ, j[1].dest.node_id, m[1].chan.dest.node_id))
...
if __name__ == "__main__":
    ...
    DS.load_rxtx('/home/aleksei/Nextcloud/Documents/TTY/WORK/mmWave/Simulations/WI/Class@60GHz/TEST_60/Class@60GHz.TEST_60.sqlite')
    DS.load_path()
    DE = distanced_delta_hist_extractor(DS, range=(0, 18), histbins=50, frac=0.85)
    DE.build(txgrp=-1, rxgrp=-1, thresh=-115, start_typ='nolink', sw_typ='link')
    DE.CDF_mutate()
    DE.plot_hist()
    exit()