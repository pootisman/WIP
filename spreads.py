...
class DelaySpreadHist:
    ...
    def export(self, rxgrp: list = [-1], txgrp: list = [-1], xlims: tuple = (0, 10), ylims: tuple = (0, 10), figdims: tuple = (640, 480),
        ...
        for i in rms_hist.bins.items():
            ...
            eebars.append(np.sqrt(np.var(np.asarray(i[1])*1e9)))
        ...
        mpl.xticks(ticks)
        mpl.errorbar(xebars, yebars, yerr=eebars, fmt='o', capthick=2)
        mpl.tight_layout()
...
class AzSpreadHist:
    ...
    def export(self, rxgrp: list = [-1], txgrp: list = [-1], xlims: tuple = (0, 10), ylims: tuple = (0, 10), figdims: tuple = (640, 480),
            ...
            if i[1].setid in txgrp or txgrp[0] == -1:
                ...
                for j in i[1].chans_to_pairs.items(destgrprange=rxgrp):
                    ...
                    if j[0].setid in rxgrp or rxgrp[0] == -1:
                        ...
                        weights_vector = []
                        for k in j[1].paths.items():
                            azimuth_vector.append(k[1].aod)
                            weights_vector.append(k[1].pow)
                        mw = sum(weights_vector)
                        md = min(azimuth_vector)
                        weights_vector = [w/mw for w in weights_vector]
                        azimuth_vector = [d - md for d in azimuth_vector]

                        rms = np.sqrt((np.sum([w * (np.average(a=azimuth_vector, weights=weights_vector) - d)**2 for d, w
                                               in zip(azimuth_vector, weights_vector)])))
                        rms_hist.append(j[1].dist, rms)
        ...
        for i in rms_hist.bins.items():
            if not isinstance(i[1], list):
                bars.append(mpp.Rectangle((i[0][0], 0.0), i[0][1] - i[0][0], i[1]))
            else:
                bars.append(mpp.Rectangle((i[0][0], 0.0), i[0][1] - i[0][0], np.mean(i[1])))

            ticks.append(i[0][0])

            xebars.append((i[0][0] + i[0][1]) / 2.0)
            yebars.append(np.mean(i[1]))
            eebars.append(np.sqrt(np.var(np.asarray(i[1]))))
        ...
        ax.set_ylim(0, 360)
        mpl.xlabel('Distance [m]')
        mpl.ylabel('RMS angular spread [deg.]')
        mpl.xticks(ticks)
        mpl.errorbar(xebars, yebars, yerr=eebars, fmt='o', capthick=2)
        mpl.tight_layout()
        ax.add_collection(rms_bc)
